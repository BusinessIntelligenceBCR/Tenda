# ==============================================================================
# SCRIPT 1 INCREMENTAL: COMPLEMENTAR BASE LOCAL PARQUET
# ==============================================================================
import os
import time
import requests
...
# --- VERIFICAÇÃO DO PONTO DE PARTIDA (INCREMENTAL) ---
df_antigo = pd.DataFrame()
data_inicio_dinamica = "2026-02-01"

if os.path.exists(BASE_PARQUET):
    print(f"📖 Arquivo local encontrado. Identificando último ponto de parada...")
    df_antigo = pd.read_parquet(BASE_PARQUET)
    if not df_antigo.empty:
        # Pega a data do ticket mais recente e adiciona 1 segundo para não duplicar
        df_antigo['Data_Ordenacao'] = pd.to_datetime(df_antigo['Data_Ordenacao'])
        ultima_data = df_antigo['Data_Ordenacao'].max()
        data_inicio_dinamica = ultima_data.strftime("%Y-%m-%dT%H:%M:%SZ")
        print(f"➔ Base existente possui {len(df_antigo)} registros. Continuando a partir de: {data_inicio_dinamica}")
...
def formatar_tag(valor):
    ...
    if isinstance(valor, list):
        return " - ".join([str(v).replace("_", " ").strip().title() for v in valor if v])
...
def classificar_motivo_tenda(ticket):
    ...
    if improdutivo and str(improdutivo).strip().lower() not in ["none", "null", ""]: 
        return f"Improdutivo > {formatar_tag(improdutivo)}"
    return "Não classificado"

# --- EXECUÇÃO ---
url = f"{url_base}/api/v2/search/export.json"
query = f"created>={data_inicio_dinamica} created<={DATA_FIM}"
...
try:
    while True:
        ...
        if response.status_code != 200:
            print(f"❌ Parada da API: {response.text}")
            break
        ...
        for t in tickets:
            ...
            for id_cpf in IDS_CPFS:
                ...
                if len(num) == 11: cpfs_encontrados.append(num)
            ...
            if cpfs_encontrados:
                ...
                for cpf_valido in set(cpfs_encontrados):
                    tickets_novos.append({
                        'CPF_Limpo': cpf_valido, 'Ticket ID': t['id'],
                        'Data_Ordenacao': data_completa, 'Canal de Entrada': canal_bruto, 'Motivo de Contato': motivo_bruto
                    })
                    
        meta = dados.get('meta', {})
        if not meta.get('has_more'): break
        params['page[after]'] = meta.get('after_cursor')
        print(f"🔄 Buscando novos pacotes... Total nesta rodada: {len(tickets_novos)} tickets...")

except Exception as e:
    print(f"⚠️ Interrupção capturada: {e}")

# Se encontrou novos dados, concatena com a base antiga e salva
if tickets_novos:
    df_novos = pd.DataFrame(tickets_novos)
    df_novos['Data_Ordenacao'] = pd.to_datetime(df_novos['Data_Ordenacao'])
    
    df_consolidado = pd.concat([df_antigo, df_novos], ignore_index=True)
    # Remove eventuais duplicados por ID de Ticket e CPF garantindo integridade
    df_consolidado = df_consolidado.drop_duplicates(subset=['Ticket ID', 'CPF_Limpo'])
    
    # Volta o campo de data para string antes de salvar no Parquet para o Script 2 ler
    df_consolidado['Data_Ordenacao'] = df_consolidado['Data_Ordenacao'].dt.strftime("%Y-%m-%d %H:%M:%S")
    df_consolidado.to_parquet(BASE_PARQUET, index=False)
    print(f"💾 Sucesso! Foram agregados novos tickets. Total da base local: {len(df_consolidado)} registros.")
else:
    print("ℹ️ Nenhum ticket novo foi adicionado. A base continua com o volume anterior.")